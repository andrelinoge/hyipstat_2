            ...
            reader.onload = function(e){
                if (!document.jcrop.options.image_loaded) {
                    ...
                    document.jcrop.options.cropbox_image.on('load', function () {
                        ...
                        document.jcrop.options.cropbox_image.removeProp('style');

                        $this = $(this);
                        cropbox_wrapper_with   = $this.parents(".cropbox_wrapper:first").width();
                        cropbox_wrapper_height = $this.parents(".cropbox_wrapper:first").height();
                        ...
                        document.jcrop.options.cropbox_image.Jcrop({
                            ...
                            maxSize: [cropbox_wrapper_with, cropbox_wrapper_height],
                            boxWidth: cropbox_wrapper_with,
                            boxHeight: cropbox_wrapper_height
                        }, function() {
                            jcrop_api = this;
                        });
                    });
                ...
                if (document.jcrop.options.previewbox_image.length > 0) {
                    document.jcrop.options.previewbox_image.attr('src', e.target.result);
                }
            };
